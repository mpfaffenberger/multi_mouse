...
namespace masesk {
	int inet_pton_i(int af, const char *src, void *dst)
	{
	...
	const char *inet_ntop_i(int af, const void *src, char *dst, socklen_t size)
	{
	struct sockaddr_storage ss;
	...
	return (WSAAddressToStringA((struct sockaddr *)&ss, sizeof(ss), NULL, (LPSTR) dst, &s) == 0)?
			dst : NULL;
	}
	...
	public:
		void socketListen(const std::string &channelName, int port, std::function<void (const std::string &data)> callback) {
			...
			{
				inet_ntop_i(AF_INET, &client.sin_addr, host, NI_MAXHOST);
		...
		void socketConnect(const std::string &channelName, const std::string &ip, std::uint16_t port) {
			...
			hint.sin_family = AF_INET;
			hint.sin_port = htons(port);
			inet_pton_i(AF_INET, ip.c_str(), &hint.sin_addr);
